...
2. 如果指定了语言，则通过兼容 OpenAI Chat Completions 的接口进行翻译。服务地址和模型均可配置，同一套代码可用于 Grok2API、OpenAI GPT 和 xAI Grok。
...
```sh
cp ".dev copy.example" .dev.vars   # 然后填入 LLM_API_KEY="..."
```

`.dev.vars` 保存本地 LLM 配置。它已被 git 忽略（忽略 `.dev.vars*`，保留 `.dev.vars.example`）。

Worker 使用以下变量：

| 变量 | 示例 | 是否为密钥 |
| --- | --- | --- |
| `LLM_BASE_URL` | `https://grok.6661993.xyz/v1` | 否 |
| `LLM_MODEL` | `grok-4.5` | 否 |
| `LLM_API_KEY` | 服务商 API Key | 是 |

使用 GPT 时可将 `LLM_BASE_URL` 设为 `https://api.openai.com/v1`，使用 xAI Grok 时可设为 `https://api.x.ai/v1`，并把 `LLM_MODEL` 改为该服务商账号可用的模型。也可以直接填写完整的 `/chat/completions` 地址。
